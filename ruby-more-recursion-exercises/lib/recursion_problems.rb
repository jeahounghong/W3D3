...
def recursive_sum(array)
  ...
  return array[0] + recursive_sum(array.drop(1))
end
...
def sorted?(array)
  ...
  return (array[0] <= array[1]) && sorted?(array.drop(1))
  # Your code here
end
...
def update_order_total(total, updates) 
  # Your code here
  #el = total
  return total if updates.length == 0
  next_item = updates[0]
  add = next_item[:price] if next_item[:status] == "add"
  add = -1*next_item[:price] if next_item[:status] == "remove"
  add = 0 if next_item[:status] == nil
  total += add
  return update_order_total(total, updates.drop(1))
...
def recursive_solution(data, by_completion = nil)
  if by_completion == nil
    by_completion = { complete: [], incomplete: [] }
  end
  if data.length == 0
  ...
  return recursive_solution(data.drop(1), by_completion)
end
